...
1. 최종결과에서 받은 "workRange"(기획,개발,디자인)를 파악합니다. 필요한 업무에 대한 맨먼스만 산정합니다. 예) 기획 혹은 디자인만 필요한 경우 frontend와 backend의 맨먼스는 0이 됩니다.
...
2. designer와 planner의 기간은 최소로 잡으세요.
3. 개발을 할때는 오픈소스 라이브러리를 적극적으로 활용한다고 가정하세요.
4. 1인 개발자 기준으로 가격을 산정하세요.
5. 기획, 디자인만 필요한 경우에는 개발 (프론트엔드와 백엔드)에 대한 맨먼스는 0으로 계산하세요.
